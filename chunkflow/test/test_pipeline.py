...
class TestInferencePipeline(unittest.TestCase):
    def setUp(self):
        ...
        self.output_bbox = Bbox.from_slices(tuple(slice(c, i-c) for i,c in 
                                             zip(self.input_size, 
                                                 self.cropping_margin_size)))
        ...
        CloudVolume.from_numpy(np.transpose(self.img), vol_path=self.input_volume_path)
        ...
        input_mask_vol = CloudVolume(self.input_mask_volume_path, mip=self.input_mask_mip)
        ...
        self.thumbnail_volume_path = os.path.join(self.output_volume_path, 'thumbnail')
